<?php require '../../php/config.php';$na=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM atasan where c_atasan='$_GET[atasan]' "));$nw=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM pimatasan where c_pimatasan='$_GET[ortu]' ")); ?>
    ...
    <?php /*chatdisini*/ 
    $uc=mysqli_query($con,"UPDATE chat SET sg='y' where c_atasan='$_GET[atasan]' and c_pimatasan='$_GET[ortu]' ");
    $chat=mysqli_query($con,"SELECT * FROM chat where c_atasan='$_GET[atasan]' and c_pimatasan='$_GET[ortu]' order by at desc ");while($chatnya=mysqli_fetch_array($chat)){
      if($chatnya['w_g']=='g'){?>
          ...
          <?php if($na['foto']==NULL){ ?>
                <img src="<?php echo $base; ?>php/img/userpng.png" class="direct-chat-img" alt="User Image">
              <?php }else { ?> 
      ...
      <?php } else { ?>
            <div class="direct-chat-msg">
              ...
              <img class="direct-chat-img" src="<?php echo $base; ?>php/img/userpng.png" alt="message user image"><!-- /.direct-chat-img -->
              <div class="direct-chat-text">
                <?php echo $chatnya['pesan']; ?>
              </div><!-- /.direct-chat-text -->
            </div>
    ...
    <?php } ?>
        ...
        <?php //echo $_GET['atasan'].'<br>'.$_GET['pimatasan']; ?>            
