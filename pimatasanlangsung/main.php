<?php require '../php/config.php'; session_start(); if(empty($_SESSION['c_pimatasan'])){header('location:../login');} $or=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM pimatasan where c_pimatasan ='$_SESSION[c_pimatasan]' ")); $aparatur=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM aparatur where c_aparatur='$or[c_aparatur]' ")); $bagian=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM bagian where c_bagian='$aparatur[c_bagian]' ")); $pel=mysqli_num_rows(mysqli_query($con,"SELECT * FROM pelanggaran where c_aparatur='$or[c_aparatur]' ")); $totalpel=mysqli_fetch_array(mysqli_query($con,"SELECT sum(bobot) as total FROM pelanggaran where c_aparatur='$or[c_aparatur]' ")); ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <title>PIMPINAN ATASAN LANGSUNG</title>
  <link rel="shortcut icon" href="<?php echo $base; ?>php/img/icons9.ico">
  ...
  <script type="text/javascript">
   var auto_refresh = setInterval(
      function(){
        $('#tampildata').load('../../view/loadpesan.php?atasan=<?php echo $_GET['q'];?>&ortu=<?php echo $_SESSION['c_pimatasan']; ?>&_=' +Math.random()).show();
      },4000);
  </script>
<?php if(empty($_GET['q'])){ ?>
  <script type="text/javascript">
   var auto_refresh = setInterval(
      function(){
        $('#pesanbaru').load('view/cekpesanbaru.php?ortu=<?php echo $_SESSION['c_pimatasan']; ?>&_=' +Math.random()).show();
      },3000);
  </script>
<?php }else{ ?>
<script type="text/javascript">
  $(document).ready(function(){
    $('#tampildata').load("../../view/loadpesan.php?atasan=<?php echo $_GET['q'];?>&ortu=<?php echo $_SESSION['c_pimatasan']; ?>");
      ...
      <div class="container">
        ...
        <div class="navbar-custom-menu navbar">
          <ul class="nav navbar-nav navbar-left">
            <li><a href="<?php echo $basewa; ?>"><i class="glyphicon glyphicon-list"></i> Pelanggaran</a></li>
            <li><a href="<?php echo $basewa; ?>atasan"><i class="glyphicon glyphicon-user"></i> Atasan Langsung</a></li>
            <li><a href="<?php echo $basewa; ?>pesan"><i class="glyphicon glyphicon-comment"></i> Percakapan <span id="pesanbaru"></span></a></li>
            <li class="user user-menu"><a href="<?php echo $base.'php/'.md5('logout'); ?>"><img src="<?php echo $base; ?>php/img/icons9.png" class="user-image" alt="User Image"><span>&nbsp;<?php echo $or['nama']; ?> <i class="glyphicon glyphicon-log-out"></i></span></a></li>
          </ul>
        </div>
        <!-- /.navbar-custom-menu -->
      </div>
