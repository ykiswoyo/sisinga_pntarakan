        ...
        <?php $_SESSION['pesan'] = '';?>
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"></span>&nbsp;Seluruh Atasan</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
<?php $smk=mysqli_query($con,"SELECT * FROM atasan order by nama asc ");$vr=1;while($akh=mysqli_fetch_array($smk)){ ?>                
                <tr>
                  ...
                  <td align="center">
              <?php $crc=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM relasichat where c_atasan='$akh[c_atasan]' and c_pimatasan='$_SESSION[c_pimatasan]' "));
                if($crc==NULL){?>
                    <a href="<?php echo $basewa; ?>w-control/<?php echo md5('tambahchat').'/'.$akh['c_atasan']; ?>" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-plus"></i> Tambah Percakapan</a>
              <?php } else{ ?>
                    <a href="<?php echo $basewa; ?>pesan/<?php echo $akh['c_atasan']; ?>/_" class="btn btn-warning btn-sm"><i class="glyphicon glyphicon-comment"></i> Kirim Pesan</a>
              <?php } ?>
                  </td>
                </tr>
<?php $vr++; } ?>
